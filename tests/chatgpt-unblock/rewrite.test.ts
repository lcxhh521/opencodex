...
import { stripSendBlocks, stripSendBlocksFromJson, stripSendBlocksFromSseLine, unlockRateLimitGate } from "../../src/chatgpt/desktop-unblock/rewrite";
...
describe("stripSendBlocks", () => {
  test("removes send locks and keeps quota display data", () => {
    const result = stripSendBlocks(blockedPayload);
...
describe("stripSendBlocksFromJson", () => {
  test("rewrites a conversation-init style body", () => {
    const rewritten = stripSendBlocksFromJson(JSON.stringify(blockedPayload));
    expect(rewritten).not.toBeNull();
    const parsed = JSON.parse(rewritten!) as typeof blockedPayload;
    expect(parsed.blocked_features).toHaveLength(1);
    expect(parsed.blocked_features[0]!.name).toBe("image_gen");
