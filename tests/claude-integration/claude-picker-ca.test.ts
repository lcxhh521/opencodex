...
function constraints(certPem: string): { critical: boolean; dnsNames: string[]; excluded: boolean } | null {
  ...
  return {
    ...
    excluded: nc.some(field => field.tag === 0xa1),
  };
}

test("picker root has a critical claude.ai-only DNS constraint; intercept root remains unconstrained", () => {
  const ca = ensurePickerCa(tempDir());
  expect(new X509Certificate(ca.certPem).subject).toContain(`CN=${PICKER_CA_COMMON_NAME}`);
  expect(constraints(ca.certPem)).toEqual({ critical: true, dnsNames: [PICKER_HOST], excluded: false });
