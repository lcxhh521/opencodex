...
import { startChatgptUnblock } from "../../chatgpt/desktop-unblock/runtime";
import { chatgptAppCommandLine, chatgptCommandLineHasRule } from "../../chatgpt/desktop-unblock/launch-watcher";
...
export function createChatgptUnblockLifecycle<T>(): ChatgptUnblockLifecycle {
  ...
  return {
    ...
    async stop() {
      ...
      if (!handle) return;
      await handle.stop();
      warnIfAppStillRouted(handle.port);
    },
  };
}

function warnIfAppStillRouted(port: number): void {
  if (process.platform !== "darwin") return;
  try {
    const app = chatgptAppCommandLine();
    if (app === null || !chatgptCommandLineHasRule(app, port)) return;
